...
import TeamCutout from "/team/round.png";
import RibbonImage from "/team/ribbon.png";

const TeamMember = ({ name, personImage, designation, size }) => {
  // Define sizes based on the size prop
  const imageSize = size === "xlarge" ? "w-72 h-72" : 
                    size === "large" ? "w-60 h-60" : "w-48 h-48"; // Adjust size for xlarge
  const cutoutSize = size === "xlarge" ? "w-96 h-96" : 
                     size === "large" ? "w-80 h-80" : "w-64 h-64"; // Adjust cutout size for xlarge
      ...
      <div className={`relative ${cutoutSize}`}>
        ...
        <div className="absolute bottom-0 left-0 w-full flex items-center justify-center z-20">
          ...
          <div className="absolute text-center z-30 flex flex-col">
            ...
            <p className="text-white text-sm mt-1">{designation}</p> {/* Designation appears below name */}
          </div>
        </div>
      </div>
